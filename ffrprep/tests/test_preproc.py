from pathlib import Path
import pytest
import shutil
from ffrprep.datasets import download_unzip_exp_data
from ffrprep.tests.test_datasets import osf_url
from ffrprep.preproc import load_data, reference_data, filter_data

def test_load_data(tmp_path):
    # Use temporary directory for testing
    dataset_path = tmp_path / "test_dataset"

    # Download and unzip the data
    download_unzip_exp_data(osf_url, dataset_path)

    # Call the function to load correct data
    data, bids_path = load_data(bids_root=dataset_path, 
                                sub_label=21, 
                                session_label=None, 
                                task_label='passive', 
                                run_label=1,)

    # Should fail if not given specific EEG file
    try:
        data, bids_path = load_data(bids_root=dataset_path)
    except FileNotFoundError:
        print('BIDSPath must point to a specific EEG file')

    # Clean up the downloaded files after the test
    shutil.rmtree(dataset_path)

def test_reference_data(tmp_path):
    # Use temporary directory for testing
    dataset_path = tmp_path / "test_dataset"

    # Download and unzip the data
    download_unzip_exp_data(osf_url, dataset_path)

    # Call the function to load correct data
    data, bids_path = load_data(bids_root=dataset_path, 
                                sub_label=21, 
                                session_label=None, 
                                task_label='passive', 
                                run_label=1,)
    
    # Call the function to re-reference the data
    referenced_data = reference_data(data, ref_channels=None)

    # Should fail if not given existing EEG file
    try:
        referenced_data = reference_data(data, ref_channels='fake_channel')
    except ValueError:
        print('Given reference channel does not exist')

    # Clean up the downloaded files after the test
    shutil.rmtree(dataset_path)


def test_filter_data(tmp_path):
    # Use temporary directory for testing
    dataset_path = tmp_path / "test_dataset"

    # Download and unzip the data
    download_unzip_exp_data(osf_url, dataset_path)

    # Call the function to load correct data
    data, bids_path = load_data(bids_root=dataset_path, 
                                sub_label=21, 
                                session_label=None, 
                                task_label='passive', 
                                run_label=1,)
    
    # Call the function to filter the data
    filtered_data = filter_data(eeg_data, high_pass=80, low_pass=1000)

    # Should fail if low-pass (upper) frequency is above
    # the Nyquist frequency of the EEG data
    try:
        filtered_data = filter_data(eeg_data, low_pass=99999)
    except ValueError:
        print('Upper edge frequency of the filter must be below '
              'the Nyquist frequency of the EEG data')

    # Clean up the downloaded files after the test
    shutil.rmtree(dataset_path)
