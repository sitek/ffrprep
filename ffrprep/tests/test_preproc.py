...
def test_epoch_count_sidecar_counts_absolute_amplitude_rejections(tmp_path):
    """--reject-mode abs drops trials (reason ABS_AMP) *before* the per-condition
    counts are taken, so the sidecar must satisfy kept + rejected == total."""
    raw, events_path = _raw_with_two_conditions_marker_and_one_spike(tmp_path)
    epochs, _ = epoch_data(
        raw, baseline=[-0.05, 0.0], events_file=str(events_path),
        tmin=-0.05, tmax=0.2, reject={"eeg": 30e-6}, reject_mode="abs",
        trial_types=["a", "b"], verbose=False,
    )
    bids_root = tmp_path / "bids"
    bids_root.mkdir()
    out_paths = save_preprocessing_outputs(
    ...
    a_path = next(p for p in out_paths if "preprocA_" in p.name)
    b_path = next(p for p in out_paths if "preprocB_" in p.name)
    assert _sidecar_counts(a_path) == (3, 1, 4)   # kept, rejected (ABS_AMP), total
    assert _sidecar_counts(b_path) == (4, 0, 4)
    with open(a_path.with_suffix(".json")) as f:
        assert json.load(f)["RejectionMode"] == "absolute-amplitude"
